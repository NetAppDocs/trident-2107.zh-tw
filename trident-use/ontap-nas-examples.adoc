...
NOTE: `useREST` 以*技術預覽*的形式提供、建議用於測試環境、而非用於正式作業工作負載。設定為時 `true`、Astra Trident將使用ONTAP 靜止API與後端進行通訊。此功能需要ONTAP 使用更新版本的版本9.8。此外ONTAP 、所使用的登入角色必須能夠存取 `ontap` 應用程式：這是預先定義的 `vsadmin` 和 `cluster-admin` 角色：
...
｛"version"：1、"storageDriverName"："ontap-nas-flexgroup"、"managementLIF"："10.0.0.1"、"dataLIF"："10.0.0.2"、"標籤"：｛"k8scluster」："test-cluster東1b"、 「後端」：「test1-ontap叢集」｝、「SVM」：「SVM NFS」、「使用者名稱」：「vsadmin」、「password」：「secret」、｝
