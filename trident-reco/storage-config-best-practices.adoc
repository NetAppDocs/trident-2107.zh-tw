...
NetApp產品組合中的每個儲存平台都有獨特的功能、無論應用程式是否為容器化的應用程式帶來好處。Trident可搭配ONTAP 下列每個主要平台使用：功能特點：功能特點、元件和E系列。沒有一個平台比其他平台更適合所有應用程式和案例、不過在選擇平台時、應考慮應用程式和管理裝置團隊的需求。
...
瞭解為Trident設定E系列儲存設備的最佳實務做法。



=== E系列磁碟集區和磁碟區群組

根據您的需求建立磁碟集區和磁碟區群組、並決定應如何將總儲存容量組織成磁碟區、並在主機之間共用。磁碟集區和磁碟區群組都由一組磁碟機組成、這些磁碟機會以邏輯方式分組、以提供一或多個磁碟區給應用程式主機。磁碟集區或磁碟區群組中的所有磁碟機都應該是相同的媒體類型。



=== E系列主機群組

Trident使用主機群組來存取其所配置的磁碟區（LUN）。根據預設、Trident會使用名為的主機群組 `trident` 除非您在組態中指定不同的主機群組名稱。Trident本身不會建立或管理主機群組。您必須先建立主機群組、才能在Trident上設定E系列儲存後端。確保主機群組中的所有Kubernetes工作節點iSCSI IQN名稱均已更新。



=== E系列快照排程

建立快照排程、並將Trident建立的磁碟區指派給快照排程、以便在所需的時間間隔內進行磁碟區備份。根據根據快照原則所擷取的快照、您可以將快照映像還原至基礎磁碟區、在磁碟區上執行復原作業。您應該使用SANtricity 「系統管理程式」來建立快照排程。



=== Snapshot一致性群組

設定快照一致性群組也適用於橫跨多個磁碟區的應用程式。一致性群組的目的是要同時擷取多個磁碟區的快照映像、以確保特定時間點的磁碟區集合複本一致。您應該使用SANtricity 「系統管理程式」來建立一致性群組。



== AWS最佳實務做法的適用範例Cloud Volumes Service

瞭解在Cloud Volumes Service AWS for Trident上設定功能的最佳實務做法。



=== 建立匯出原則

為了確保只有獲授權的節點組能夠存取透過Cloud Volumes Service 效益管理系統配置的Volume、請在建立Cloud Volumes Service 一個效益管理系統的同時、為匯出原則設定適當的規則。透過Trident在Cloud Volume Services上配置磁碟區時、請務必使用 `exportRule` 後端檔案中的參數、可讓您存取所需的Kubernetes節點。



=== 建立快照原則

為透過Cloud Volume Service配置的磁碟區建立快照原則、以確保每隔一段時間就能擷取快照。如此可保證資料定期備份、並在資料遺失或資料毀損時還原資料。您可以在Volume詳細資料頁面上選取適當的排程、為Cloud Volume Service主控的Volume設定快照原則。



=== 選擇適當的服務層級、儲存容量和儲存頻寬

適用於AWS的Cloud Volume Services提供不同的服務層級、例如標準、優質和極致。這些服務層級可滿足不同的儲存容量和儲存頻寬需求。請務必根據您的業務需求、選擇適當的服務層級。

您應該根據應用程式的特定需求、在建立磁碟區期間、選取所配置儲存設備的所需大小。在決定分配的儲存設備時、需要考量兩個因素：

* 特定應用程式的儲存需求
* 尖峰或邊緣所需的頻寬


儲存頻寬取決於服務層級與您所選的已分配容量的組合。因此、請選擇適當的服務層級和分配的容量、並將所需的頻寬納入考量。



=== 限制Trident所建立的Volume大小上限

我可以使用限制Trident在Cloud Volume Services for AWS上建立的Volume大小上限 `limitVolumeSize` 後端組態檔中的參數。如果要求的磁碟區大小高於設定值、設定此參數可確保資源配置失敗。
...
* https://docs.netapp.com/ess-11/topic/com.netapp.doc.ssm-exp-ic-lin/home.html["安裝及設定Linux"^]
