...
IMPORTANT: 如果您使用的是私有映像儲存庫、則應該新增 `/k8scsi` 適用於1.17或之前的Kubernetes版本 `/sig-storage` 若為1.17之後的Kubernetes版本、則會移至私有登錄URL結尾。使用的私有登錄時 `tridentctl` 部署、您應該使用 `--trident-image` 和 `--autosupport-image` 與搭配使用 `--image-registry`。如果您使用Trident運算子來部署Astra Trident、請確定Orchestrator CR包含在內 `tridentImage` 和 `autosupportImage` 安裝參數。
